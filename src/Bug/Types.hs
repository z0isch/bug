...
translate :: [BPoint] -> V2 Int -> [BPoint]
translate pts t = liftU2 (^+^) pts (replicate (length pts) t)

mult ::  [BPoint] -> M22 Int ->  [BPoint]
mult pts r = map (r !*) pts
...
bugs (Grid s g) = V.ifoldl' mkPs (mempty,mempty) g
    where
        ...
        f ps p = maybe (H.insert p ps) (\b' -> H.insert (H.union b' p) $ H.delete b' ps) b
            where b = headMay $ H.toList $ H.filter (getAny . foldMap (Any . (`H.member` p))) ps
        mkPs p i = \case
